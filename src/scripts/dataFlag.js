...
function dataFlagFunc(dataSetList, dataIsOs, url) {
    waitForElement(".data-table__container", ".data-table__stale-loader-container").then(() => {
        ...
        elements.forEach(function (element) {
            try {
                ...
                if (item_data) {
                    ...
                    if (!groupedSharpeData[region]) {
                        groupedSharpeData[region] = {};
                    }
                    if (!groupedSharpeData[region][delay]) {
                        groupedSharpeData[region][delay] = {};
                    }
                    if (!groupedSharpeData[region][delay][universe]) {
                        groupedSharpeData[region][delay][universe] = [];
                    }
                    groupedSharpeData[region][delay][universe].push({
                        datasetId: lastPart,
                        "OS/IS Sharpe": srText === '--' ? 'NA' : srText,
                        "Count": cntText
                    });
                } else {
                    // Handle cases where there is no item_data
                    if (!groupedSharpeData[region]) {
                        groupedSharpeData[region] = {};
                    }
                    if (!groupedSharpeData[region][delay]) {
        ...
        console.log("Grouped OS/IS Sharpe Ratios:", groupedSharpeData);
    })
}
